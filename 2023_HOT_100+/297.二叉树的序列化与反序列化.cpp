...
// 先序和中序应该可以存储，105 从前序与中序遍历序列构造二叉树
// 遍历和序列化区别导致
// 哦，一般的先序就足够了
class Codec {
public:
    ...
    TreeNode* deserialize(string data) {
        ...
        TreeNode* ans = dfs_build(_store);
        return ans;
    }

    //* 对于长字符串s进行拼接操作用 s+= " " 的效率大于 s = s + " ", 
    //* 前者复杂度为O(1), 后者为O(len_s)会爆TLE
    //* 除此以外 新建空间也会超时 
    // e.g. string dfs_traversal(TreeNode* tree, string store);
    void dfs_traversal(TreeNode* tree, string& store) {    
        if (tree == nullptr) {
            store += "null,";
            return ;
        }
        store += to_string(tree->val) + ","; //to_string !
        dfs_traversal(tree->left,  store);
        dfs_traversal(tree->right, store);
        return ;
    }
    TreeNode* dfs_build(queue<string>& _store) {
        string tmp = _store.front();
        //store.erase(store.begin());
        _store.pop();
        TreeNode* tree;
        if (tmp != "null") {
            tree = new TreeNode(stoi(tmp));
            tree->left  = dfs_build(_store);
            tree->right = dfs_build(_store); 
            return tree;
        }
        return nullptr;
    }
