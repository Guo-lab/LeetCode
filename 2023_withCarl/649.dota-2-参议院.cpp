...
public:
    string predictPartyVictory(string senate) {
        ...
        // 贪心在局部最优解是希望 ban 掉后面的对手
        queue<char> vote;

        while ((strchr (senate.c_str(), 'D') != NULL && strchr (senate.c_str(), 'R') != NULL)) {
            string tmp = senate;
            senate = "";
            ...
            while (!vote.empty()) {
                senate = vote.front() + senate;
                vote.pop();
            }
        }
        return senate.find('D') == string::npos ? "Radiant" : "Dire";
    }
};
// @lc code=end
